...
**R Source**: [EMH_mod.R](https://raw.githubusercontent.com/Dustin21/Dustin21.github.io/master/AQM/Week13_FinMod/EMH_mod.R)

For the next workshop, please do the following:

1. Set up the confusion matrix for the logistic regression. That is, the matrix showing the number of correct vs incorrect predictions.
2. Determine the classification rate, as well as the error rate.
3. Instead of reusing the training data, check your results with cross-validation (CV).
	* Set up a cross-validation loop (or find a black-box function in R)
	* Set up the confusion matrix corresponding to the cross-validated predictions
	* Determine the CV classification rate and error rate
	* Do the results coincide with the paper? How do the CV results differ from the fit on the training set? Is this meaningful?
4. State your conclusion relating to the hypothesis mentioned in class (check slides)
5. Redo the above for a "short", as apposed to the "long" as presented in the paper. Just read through the paper, and it will tell you exactly how they tested for this, then follow the same procedure that you did for the long.
...
There is much we can talk about regarding the autocorrelation (acf) and partial autocorrelation (pacf) functions. We can use these to spot non-stationarity such as trends and seasonaility, as well as to uncover the structure of ARIMA process to model the error, which is the most important aspect. Here's a [resource on ARIMA](http://sfb649.wiwi.hu-berlin.de/fedc_homepage/xplore/tutorials/xegbohtmlnode39.html) to fuel your interest.
...
Slides: [NeuralNets.pdf](https://github.com/Dustin21/Dustin21.github.io/raw/master/AQM/Week15_NeuralNets/NeuralNets.pdf)

Python source: _in the slides_

#### Assignment: 
Read over and understand the Prime Number sieve algorithm that was covered this week in Python, then re-implement the algorithm in R. The Python code is provided in the slides on slide 10. All you must do is transfer the it to R syntax and get it running in R. Cross-check to make sure both algorithms give you the same results.
